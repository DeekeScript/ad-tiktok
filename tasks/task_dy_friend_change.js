...
let task = {
    ...
    //type 0 评论，1私信
    getMsg(type, title, age) {
        if (storage.get('setting_baidu_wenxin_switch', 'bool')) {
            return { msg: type === 1 ? baiduWenxin.getChat(title, age) : baiduWenxin.getComment(title) };
        }
        ...
        while (true) {
            try {
                ...
                for (let i in contains) {
                    ...
                    DyCommon.back();
                    DyCommon.sleep(500);
                    if (DyCommon.aId('text1').descContains(T('粉丝')).isVisibleToUser(true).findOne() || DyCommon.aId('text1').textContains(T('粉丝')).isVisibleToUser(true).findOne()) {
                        Log.log("在列表页面了");
                    } else {
                        DyCommon.back();
                    }
